...
##
## Adds properties to guacamole.properties which configure the Cloudflare
## Access authentication provider.
##
associate_cloudflare() {
    if [ -z "$CLOUDFLARE_ACCESS_TEAM" ] || \
       [ -z "$CLOUDFLARE_ACCESS_AUDIENCE" ]
    then
        cat <<END
FATAL: Missing required environment variables
-----------------------------------------------------------------------------------
If using the Cloudflare Access authentication extension, you must provide each of 
the following environment variables:

    CLOUDFLARE_ACCESS_TEAMR         The name of the your Cloudflare Access Team

    CLOUDFLARE_ACCESS_AUDIENCE      The value of the 'aud' claim in the Application JWT 
END
        exit 1;
    fi
...
# Use Cloudflare Access if specified.
if [ -n "$CLOUDFLARE_ACCESS_TEAM" ]; then
    associate_cloudflare
    INSTALLED_AUTH="$INSTALLED_AUTH cloudflare"
fi
